...
刺猬选短信把本机短信导出成一个压缩包，保存在手机里。

每个卡槽对应一个手机号。导出时用这个号码作为 TXT 文件名，文件里是该卡槽的短信。条数超过 500 时拆成多批，例如 `13800138000.txt`，或 `13800138000_001.txt`、`13800138000_002.txt`。读不到号码时用 `卡槽1`、`卡槽2` 或 `未知卡槽`。这些 TXT 打进一个 zip。默认保存到系统下载文件夹，也可以改成手机上的其他文件夹。

应用不发送、不删除、不修改系统短信。以前的平台归类代码还在仓库里，打开应用后的主界面是导出。历史规格见 [`MVP_SPEC.md`](MVP_SPEC.md)。
...
- 选择保存位置。默认是下载文件夹，也可以改成手机上的其他文件夹。
- 可选授权读取本机号码，这样 TXT 文件名用卡槽号码；不授权则用卡槽 1、卡槽 2 或未知卡槽。
- 开始导出。短信按卡槽写入 TXT，每批最多 500 条，再打包成一个 zip。

下面这些页面的代码还在，但已经不是打开应用后的入口：
